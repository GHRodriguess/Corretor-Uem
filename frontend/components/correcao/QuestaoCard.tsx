...
import { calcularPontuacao, alternativasCorretas } from "../../lib/scoring";
        ...
        >
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    {questao.anulada && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 font-semibold uppercase tracking-wide">
                            Anulada
                        </span>
                    )}
                    {questao.idioma && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 font-semibold uppercase tracking-wide">
                            Idioma
                        </span>
                    )}
                </div>
                ...
                <div className="text-right">
                    ...
                    <span className="text-muted-foreground/60 text-xs"> / 6</span>
                </div>
            </div>

            {questao.anulada ? (
                <div className="flex items-center justify-center h-9 rounded-lg bg-amber-500/5 border border-amber-500/10">
                    <span className="text-xs text-amber-600/70 dark:text-amber-500/70">Pontuação automática</span>
                </div>
            ) : modo === "padrao" ? (
                <BitmaskSelector
                    value={marcado}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <div className="flex gap-1">
                            {alternativasCorretas(questao.resposta).length === 0 ? (
                                <span className="text-xs text-muted-foreground/80">Nenhuma (resposta 0)</span>
                            ) : (
                                        ...
                                        <span
                                            ...
                                            className={`text-xs px-1.5 py-0.5 rounded font-bold border ${
                                                marcouEste
                                                    ...
                                                    : "bg-muted/85 border border-border/80 text-muted-foreground"
                                            }`}
                                        >
                                            {b}
                                        </span>
                        ...
                        <span className="text-md text-muted-foreground/60 ml-auto tabular-nums">
                            = {questao.resposta}
                        </span>
                    </div>
                </div>
